...
namespace nrcv2.services
{
    public class Gtools
    {
        ...
        public async Task<List<T>> GetDataFromSqlAsync<T>(string argsql, params object[] parameters) where T:class ,new()
        {
            List<T> _data=new() ;
            try
            {
                using var db = dbf.CreateDbContext();
                    _data = await db.Set<T>().FromSqlRaw<T>(argsql, parameters).ToListAsync();
            }
            catch (Exception )
            {
             throw ;
